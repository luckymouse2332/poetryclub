...
- 首页以一张真实诗集合照为主要图像，不使用多图画廊、假作品或装饰性素材堆叠。
- 首页桌面端采用左侧暖纸介绍与右侧宽幅照片叠合的编辑布局，纸面边缘使用不规则裁切；窄屏改为自然上下排列。
- 首页下半部分由“最新诗作”和“关于回中诗社”组成，区块标题使用短印章红横线。
...
- 小于 `lg` 时，品牌单独占一行，导航使用三列或等分网格并带顶部分隔线；`lg` 起恢复单行品牌与右侧导航。品牌和当前页面通过印章红短线表达当前位置。
- `PageContainer`：统一最大宽度、响应式页边距和可选窄 / 阅读宽度。
- `PageHeader`：eyebrow、标题、描述和可选 actions；标题层级由调用方指定。
- `Section`：统一垂直节奏，可包含区块标题、描述和 actions。
- 首页首屏：移动端为介绍在上、照片在下；`lg` 起左侧纸面覆盖照片一部分，照片延伸至右侧页面边缘。主操作和“查看全部”采用文字链接，箭头在 hover / focus-visible 时轻微右移。
- 首页最新诗作：日期、诗名、作者按响应式网格排列。反馈只作用于诗名文字与下划线，不给整行增加背景或阴影。
- 首页关于区：与最新诗作并列或上下排列，正文和收录标准保持长文可读宽度。
