...
- 独立 About 页：以“序言 → 过去 → 现在 → 未来 → 附录”的诗社沿革组织内容。序言说明来源与继续维护的原因；“四次迁徙”以年份和正文的左右关系记录历史；“现在”使用三栏说明网站今天承担的角色；未来方向使用无卡片的两列编辑网格；更新记录降为页面末尾从 M1 起按时间排列的主要阶段。各章节通过不同排版节奏区分，不使用时间轴圆点、营销式大文案或密集卡片。
