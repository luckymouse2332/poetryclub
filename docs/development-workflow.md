...
本文定义回中诗社从任务编号、工作分支、提交信息到正式发布的统一规则。规则从 `CHORE-1` 完成后适用于新工作。已有提交、标签和任务文件作为历史记录保留，不要求改名或重写。已经合入 `master` 的开发分支不作为历史档案保留；要继续进行新工作，必须从最新 `master` 创建符合新规则的新分支。
...
| 名称 | 示例 | 用途 |
| --- | --- | --- |
| 任务编号 | `M7`、`M4.1`、`BUG-4`、`CHORE-1` | 说明为什么做、范围和验收条件 |
...
产品功能、架构能力和较大的工程阶段使用 `M<阶段>` 或 `M<阶段>.<序号>`。阶段根任务可以直接使用 `M7`；同一阶段需要拆分后续任务时使用带序号形式。阶段和序号都是十进制整数，点号表示层级，不表示小数，因此 `M3.10` 位于 `M3.9` 之后。

- `M7` 表示 M7 阶段的根任务。
- `M4.0` 表示采用带序号形式记录的 M4 阶段基线任务。
- `M4.1`、`M6.4` 表示阶段内按计划推进的独立任务。
- 启用 `M5.0` 前，必须先在任务清单中写明 M5 的整体目标。仅因为上一项是 `M4.9`，不能自动开始 M5。

新阶段可以使用 `Mx` 根任务，也可以在一开始就拆成 `Mx.0` 基线与后续序号任务。创建 `Mx` 或 `Mx.0` 分支前，必须先在 `docs/tasks/README.md` 的“阶段定义”中写明该阶段的整体目标；阶段定义不存在时，规范检查会拒绝该分支。
...
分支名只能使用小写 ASCII 字母、数字和连字符。普通工作分支属于短期分支，合入 `master` 后必须在同一次工作收尾中删除本地、`origin` 和 `mirror` 的同名分支。删除前必须确认分支 tip 已经可以从 `master` 到达；尚未合入、仍有开放 PR 或废弃决定不明确的分支必须保留。禁止为了清理分支而擅自创建合并提交。已经共享的分支禁止强制推送；需要整理历史时，在首次推送前完成。

仓库的常态分支只包括 `master`、`release/v*`，以及尚未合入且仍在进行的短期工作分支。功能、修复、文档和维护分支合入后不长期保留。
...
热修复完成后合回 `master`，发布对应 PATCH 版本，并同步两个远程。已完成的热修复分支按普通开发分支删除；`release/v*` 分支作为版本准备记录保留，但发布完成后不得继续加入新提交，后续修复必须使用新的 release 或 hotfix 分支。
...
10. 删除已合入的普通和 hotfix 分支，保留 `release/v*` 分支。未合并分支不得为清理目的擅自合入；已发布标签永不移动，需要修复时发布新版本。
